...
namespace {
  ...
  using node_nbrs_type = std::vector<std::pair<std::string, std::string>>;
...
namespace diskann {
  std::string cut_edge(const std::string& input) {
    size_t pos = input.find_last_of('_');
    if (pos != std::string::npos) {
      std::string res = input.substr(pos + 1);
      return res;
    } else {
      diskann::cout << "No edge found in the string " << input << std::endl;
    }
...
#pragma omp critical
  ...
  template<typename T, typename TagT>
 void PQFlashIndex<T, TagT>::load_cache_list_kv(std::vector<uint32_t> &node_list){
    for (auto node : node_list) {
      auto coord = json::parse(this->kv->get("node_" + std::to_string(node))).template get<std::vector<T>>();
      node_nbrs_type res;
      this->kv->prefix_scan("edge_" + std::to_string(node) +"_", res);

      auto meta = std::find_if(res.begin(), res.end(), [&](std::pair<std::string, std::string> &nbr){
      return  nbr.first == "edge_" + std::to_string(node) + "_meta";});
      res.erase(meta);

      T * cached_coord = new T[coord.size()];
      memcpy(cached_coord, coord.data(), coord.size() * sizeof(T));
      coord_cache.emplace(node, coord.data());

      std::vector<_u32> _nbrs;
      std::transform(res.begin(), res.end(), std::back_inserter(_nbrs), [](const std::pair<std::string, std::string>& p) { return std::stoul(cut_edge(p.first)); });
      _u32 * nbrs = new _u32 [_nbrs.size()];
      memcpy(nbrs, _nbrs.data(), _nbrs.size() * sizeof(_u32));
      nhood_cache.emplace(node, std::make_pair(res.size(), nbrs));
    }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  template<typename T, typename TagT>
  void PQFlashIndex<T, TagT>::cache_bfs_levels_kv(
    ...
    while ((node_list.size() + cur_level->size() < num_nodes_to_cache) &&
      ...
      using node_nbrs_type = std::vector<std::pair<std::string, std::string>>;
      std::map<unsigned, node_nbrs_type> cur_batch_nbrs;
      for(auto node_to_expand : nodes_to_expand) {
        node_nbrs_type res;
        this->kv->prefix_scan("edge_" + std::to_string(node_to_expand) +"_", res);
        cur_batch_nbrs.emplace(node_to_expand, res);
      }

      for(auto node_nbr : cur_batch_nbrs){
       auto meta = std::find_if(node_nbr.second.begin(), node_nbr.second.end(), [&](std::pair<std::string, std::string> &nbr){
        return  nbr.first == "edge_" + std::to_string(node_nbr.first) + "_meta";});
       node_nbr.second.erase(meta);

       for(auto nbr : node_nbr.second){
          if (std::find(node_list.begin(), node_list.end(), std::stoull(cut_edge(nbr.first))) ==
              node_list.end()) {
            cur_level->insert(std::stoull(cut_edge(nbr.first)));
          }
 ...
 template<typename T, typename TagT>
 int PQFlashIndex<T, TagT>::load_kv(const char *index_prefix, _u32 num_threads,
                                    bool new_index_format) {
    this->single_index_file = false;
    ...
    std::vector<_u64> output_file_meta =
        json::parse(kv->get("output_file_meta")).get<std::vector<_u64>>();
    disk_nnodes = output_file_meta.at(0);
    disk_ndims = output_file_meta.at(1);
    medoid_id_on_file = output_file_meta.at(2);
    max_node_len = output_file_meta.at(3);
    nnodes_per_sector = output_file_meta.at(4);
    data_dim = disk_ndims;
    ...
    this->num_frozen_points = output_file_meta.at(5);
    file_frozen_id = output_file_meta.at(6);
...
// read index metadata
  ...
  size_t PQFlashIndex<T, TagT>::cached_beam_search_ids(
    ...
    if (useKv) {
      this->disk_iterate_to_fixed_point_kv(query, (_u32) l_search, (_u32) beam_width,
  ...
  template<typename T, typename TagT>
  void PQFlashIndex<T, TagT>::disk_iterate_to_fixed_point_kv(
    ...
    std::vector<std::pair<unsigned, std::vector<unsigned >>> frontier_nhoods;
    ...
    while (k < cur_list_size) {
      ...
      if (!frontier.empty()) {
        ...
        for (_u64 i = 0; i < frontier.size(); i++) {
          auto                    id = frontier[i];
          node_nbrs_type res;
          this->kv->prefix_scan("edge_" + std::to_string(id) +"_", res);

          auto meta = std::find_if(res.begin(), res.end(), [&](std::pair<std::string, std::string> &nbr){
            return  nbr.first == "edge_" + std::to_string(id) + "_meta";});
          res.erase(meta);

          std::vector<_u32> _nbrs;
          std::transform(res.begin(), res.end(), std::back_inserter(_nbrs), [](const std::pair<std::string, std::string>& p) { return std::stoul(cut_edge(p.first)); });
          frontier_nhoods.emplace_back(std::make_pair(id, _nbrs));
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        _u64      nnbrs = frontier_nhood.second.size();

        T        *node_fp_coords = json::parse(this->kv->get("node_" + std::to_string(frontier_nhood.first))).template get<std::vector<T>>().data();
        assert(data_buf_idx < MAX_N_CMPS);

        float cur_expanded_dist = dist_cmp->compare(query, node_fp_coords,
                                                    (unsigned) aligned_dim);
        bool  exclude_cur_node = false;
        if (exclude_nodes != nullptr) {
        ...
        if (!exclude_cur_node) {
          ...
          if (coord_map != nullptr) {
            coord_map->insert(
                std::make_pair(frontier_nhood.first, node_fp_coords));
          }
          full_retset.push_back(
              Neighbor(frontier_nhood.first, cur_expanded_dist, true));
        }
        unsigned *node_nbrs = frontier_nhood.second.data();
